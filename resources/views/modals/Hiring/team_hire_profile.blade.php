    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body" style="height: auto; overflow-y: auto; overflow-x: hidden;">
                @if($events)
                <form action="{{route('freelancer.hire')}}" method="POST" id="hire-team-from-profile-{{$uniqueId}}">
                    @csrf
                    ...
                    <div id="hire-step-2-{{ $uniqueId }}" style="max-height: 85dvh; overflow-y: auto; overflow-x: hidden; display:none;">
                        ...
                        <div class="d-flex mb-4 align-items-center">
                            ...
                            <div class="ms-3">
                                ...
                                <small class="text-muted mb-2 text-truncate">{{ $team->description }}</small>
                                <div class="d-flex align-items-center">
                                    @if($team->avg_rating != 0)
                                    <span class="text-warning">⭐</span>
                                    <small class="fw-bold ms-1">{{ number_format($team->avg_rating, 1) }}</small>
                                    <small class="text-muted ms-2">{{$team->reviews()->where('reviewee_role', 'team')->count()}} Reviews</small>
                                    @else
                        ...
                        <div class="list-group mb-4">
                            <div class="list-group-item d-flex justify-content-between align-items-center" style="background-color: #EEEEEE;">
                                ...
                                <span class="{{ $team->hasMinimumMemberships() ? 'text-success' : 'text-danger' }}">
                                    {{ $team->hasMinimumMemberships() ? 'Available' : 'Not Available' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <!-- Computed Fee -->
                        <p class="fw-bold" id="recomm-computed-fee-<?php echo $uniqueId; ?>">Package Fee: ₱{{ $team->package_price }}</p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.close-form-hire').forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                document.getElementById('hire-team-from-profile-' + uniqueId).reset();
                document.getElementById('recomm-computed-fee-<?php echo $uniqueId; ?>').innerHTML = 'Computed Fee: ₱ 0.00';
            });
        });


        const form = document.getElementById('hire-team-from-profile-<?php echo $uniqueId; ?>');
